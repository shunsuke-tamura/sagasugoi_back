...
function getProxyUriFromDevConnString(connectionString) {
  ...
  if (connectionString.search("DevelopmentStorageProxyUri=") !== -1) {
    ...
    for (
      var _i = 0, matchCredentials_1 = matchCredentials;
      _i < matchCredentials_1.length;
      _i++
    ) {
...
function extractConnectionStringParts(connectionString) {
  ...
  if (
    ...
    if (!blobEndpoint) {
      ...
      defaultEndpointsProtocol = getValueInConnString(
        connectionString,
    ...
    var accountSas = getValueInConnString(
      connectionString,
    ...
    if (!blobEndpoint) {
      ...
      throw new Error(
        "Invalid AccountName in the provided SAS Connection String"
      );
    }
    return {
      kind: "SASConnString",
